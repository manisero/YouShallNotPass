...
{
    public class AtLeastNValidationErrorFormatter : IValidationErrorFormatter<AtLeastNValidation.Error,
                                                                              IEnumerable<IValidationErrorMessage>>
    {
        public IEnumerable<IValidationErrorMessage> Format(
            AtLeastNValidation.Error error,
...
namespace Manisero.YouShallNotPass.ErrorMessages.FormatterRegistration
{
    ...
    {
        private static readonly Action<IValidationErrorFormattingEngineBuilder<IEnumerable<IValidationErrorMessage>>> AtLeastN
            = x => x.RegisterErrorOnlyFormatter(new AtLeastNValidationErrorFormatter());
    }
}
